...
redis:set(ADHAM..":NameBot:",BOT_NAME)
redis:hset(ADHAM..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3ADHAM_INSTALL","Yes")
info = {}
...
ADHAM = Token:match("(%d+)")
our_id = tonumber(ADHAM)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(ADHAM..":UserNameBot:")
SUDO_ID = tonumber(redis:get(ADHAM..":SUDO_ID:"))
SUDO_USER = redis:hgetall(ADHAM..'username:'..SUDO_ID).username
version = redis:get(ADHAM..":VERSION")
DataCenter = redis:get(ADHAM..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..ADHAM..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(ADHAM..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(ADHAM..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(ADHAM..'group:add'..msg.chat_id_) then 
...
local lock_bots = redis:get(ADHAM..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(ADHAM..'lock_bots_by_kick'..msg.chat_id_) then
...
if msg.GroupActive and redis:get(ADHAM..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(ADHAM..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(ADHAM..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(ADHAM..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.ADHAM then 
if msg.text and ScriptFile.iADHAM then
for k, ADHAM in pairs(ScriptFile.ADHAM) do
local SearchText = msg.text:match(ADHAM)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iADHAM(msg,{msg.text:match(ADHAM)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",ADHAM.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dADHAM then
if ScriptFile.dADHAM(msg) == false then
return false
end
...
if Plug.ADHAM then 
if msg.text and Plug.iADHAM then
for k, ADHAM in pairs(Plug.ADHAM) do
local SearchText = msg.text:match(ADHAM)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(ADHAM..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(ADHAM..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iADHAM(msg,{msg.text:match(ADHAM)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",ADHAM.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source ADHAM")
end 

end
else
print("The File Script.lua Not Runing in The Source ADHAM")
...
function tdcli_update_callback(data)
	...
	redis:srem(ADHAM..'users',msg.sender_user_id_)
	end
...
download(stk,32)
	...
	if redis:get(ADHAM..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(ADHAM..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(ADHAM..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(ADHAM..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(ADHAM..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(ADHAM..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(ADHAM..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(ADHAM..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(ADHAM..'group:add'..data.chat_.id_) then
	redis:set(ADHAM..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	redis:mset(
	ADHAM..'group:name'..k,IDS.Title,
	ADHAM..'num_msg_ADHAM'..k,5,
	ADHAM..'group:add'..k,true,
	ADHAM..'lock_link'..k,true,
	ADHAM..'lock_spam'..k,true,
	ADHAM..'lock_webpage'..k,true,
	ADHAM..'lock_markdown'..k,true,
	ADHAM..'lock_flood'..k,true,
	ADHAM..'lock_bots'..k,true,
	ADHAM..'mute_forward'..k,true,
	ADHAM..'mute_contact'..k,true,
	ADHAM..'mute_document'..k,true,
	ADHAM..'mute_inline'..k,true,
	ADHAM..'lock_username'..k,true,
	ADHAM..'replay'..k,true
	)
	redis:sadd(ADHAM..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(ADHAM..'username:'..ID,'username',user)
	redis:sadd(ADHAM..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(ADHAM..'username:'..ID,'username',user)
	redis:sadd(ADHAM..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(ADHAM..'username:'..ID,'username',user)
	redis:sadd(ADHAM..'owners:'..k,ID)
	end
	end
	end
	...
	redis:srem(ADHAM..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(ADHAM..'username:'..data.user_.id_).username
	...
	redis:hset(ADHAM..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
